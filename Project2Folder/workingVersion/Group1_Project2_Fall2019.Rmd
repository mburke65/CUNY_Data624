...
- b) RSME & R^2 Residuals Function
- c) Multiple Linear Regression
- d) Stepwise - Both, Forward, Backward 
- e) GLM
...
rsme_rsquared <- function(Y.Test, residuals.model){
  rmse.res <- round(sqrt(mean(residuals.model$PH^2)),4)
 

  total.sum.squares <- sum((Y.Test$PH -mean(Y.Test$PH))^2)
  residual.sum.squares <- sum(residuals.model$PH^2)
  r.squared <- round((residual.sum.squares/ total.sum.squares),4)

  return.df<- data.frame(cbind(rmse.res, r.squared))
  colnames(return.df) <- c("rsme", "r.squared")
...
```{r warning = F, message = F}
pred.lm <- predict( linear,X.Test)
residuals.lm <-Y.Test - pred.lm
rsme_rsquared(Y.Test,residuals.lm)
```
...
```{r}
pred.step <- predict( step.linear,X.Test)
residuals.step <-Y.Test - pred.step
rsme_rsquared(Y.Test,residuals.step)
```
...
```
```{r message = F, warning = F}
pred.glm <- predict( glm,X.Test)
residuals.glm <-Y.Test - pred.glm
rsme_rsquared(Y.Test,residuals.glm)
```
...
residuals.pls <-Y.Test - pred.pls
rsme_rsquared(Y.Test,residuals.pls)
```
...
rf.control <- trainControl(method="repeatedcv", number=5, repeats=10,  search="random")
set.seed(143)
...
```{r}
pred.rf <- predict( random.forest,X.Test)
residuals.rf <-Y.Test - pred.rf
rsme_rsquared(Y.Test,residuals.rf)
```
...
residuals.knn <-Y.Test - pred.knn
rsme_rsquared(Y.Test,residuals.knn)
```
...
```{r warning = F, message = F}
models <- list('linear.model' = linear,
               'step.linear.model' = step.linear,
               'glm.model' = glm,
               'pls.model' = pls,
               'random.forest.mode' = random.forest,
               'knn.model' = knn)
...
- random forest performs the best: highest R2, lowest mae and rsme
